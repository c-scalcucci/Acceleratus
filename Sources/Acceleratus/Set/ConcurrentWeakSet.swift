...
public class ConcurrentWeakSet<T: AnyObject & Hashable> : ConcurrentObject {
    public typealias Element = T

    public private(set) var mutex = SharedRecursiveMutex()

    public private(set) var _dataSource = Set<WeakHashablePointerContainer<Element>>()
    ...
    public init<S: Sequence>(_ c: S) where S.Iterator.Element == T {
        c.forEach({
            self._dataSource.insert(WeakHashablePointerContainer($0))
        })
    }
    ...
    public func insert(_ newMember: ConcurrentOrderedSet<Element>.Element) -> (inserted: Bool, memberAfterInsert: ConcurrentOrderedSet<Element>.Element) {
        ...
        self.sanitize()
        let result = self._dataSource.insert(WeakHashablePointerContainer(newMember))

        if result.inserted, let oldObject = result.memberAfterInsert.object {
    ...
    public func append<S>(contentsOf newElements: S) where S : Sequence, Element == S.Element {
        ...
        self.sanitize()
        newElements.forEach({ self._dataSource.insert(WeakHashablePointerContainer($0)) })
    }
    ...
    public func remove(_ object: Element) {
        ...
        self._dataSource.removeFirst(where: {
            $0.object.hashValue == object.hashValue
        })
    }
